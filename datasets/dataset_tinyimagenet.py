...
def load_train(dataset, num_per_class, distributed, batch_size, workers, 
    ...
    if dataset == 'cifar10':
        ...
        train_transform = obtain_aug(dataset, data_aug, aug_plus)
        train_dataset = torchvision.datasets.CIFAR100(data_path_dict['cifar100'], train=True, transform=train_transform, download=True)
    else:
...
def load_val_loader(dataset, batch_size, workers):
    ...
    if dataset == 'cifar10':
        val_dataset = torchvision.datasets.CIFAR10(data_path_dict['cifar10'], train=False, transform=val_transform, download=True)
    elif dataset == 'cifar100':
        val_dataset = torchvision.datasets.CIFAR100(data_path_dict['cifar100'], train=False, transform=val_transform, download=True)
    else:
        valdir = os.path.join(data_path_dict[dataset], 'val')
